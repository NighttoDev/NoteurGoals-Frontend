...
export const uploadFiles = (formData: FormData, onProgress: (progress: number) => void) => {
...
export const linkFileToGoal = async (fileId: number, goalId: number) => {
  const response = await apiClient.post(`/api/files/${fileId}/goals`, { goal_id: goalId });
  return response.data;
};

export const unlinkFileFromGoal = async (fileId: number, goalId: number) => {
  const response = await apiClient.delete(`/api/files/${fileId}/goals/${goalId}`);
  return response.data;
};

export const linkFileToNote = async (fileId: number, noteId: number) => {
  const response = await apiClient.post(`/api/files/${fileId}/notes`, { note_id: noteId });
  return response.data;
};

export const unlinkFileFromNote = async (fileId: number, noteId: number) => {
  const response = await apiClient.delete(`/api/files/${fileId}/notes/${noteId}`);
  return response.data;
};
