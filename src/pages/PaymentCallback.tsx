    ...
    useEffect(() => {
        const verifyPayment = async () => {
            ...
            if (Object.keys(params).length === 0 || !params.vnp_TxnRef) {
                setError('Dữ liệu trả về không hợp lệ hoặc giao dịch đã bị hủy.');
            ...
            try {
                ...
                setMessage('Xác thực thành công! Gói của bạn đã được kích hoạt.');
                ...
                const errorMessage = err.response?.data?.message || 'Giao dịch không thành công. Vui lòng thử lại.';
    ...
    return (
        <div style={{
            ...
            <h2 style={{ color: error ? '#d9534f' : '#5cb85c', marginBottom: '15px' }}>
                {error ? 'Đã xảy ra lỗi' : 'Đang xử lý...'}
            </h2>
            <p style={{ fontSize: '1.1rem', color: '#555' }}>
                {error || message}
            </p>
            <p style={{ marginTop: '20px', color: '#777' }}>
                Trang sẽ tự động chuyển hướng sau vài giây.
            </p>
        </div>
    );
};
