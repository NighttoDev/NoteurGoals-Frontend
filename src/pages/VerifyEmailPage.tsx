    ...
    return (
        <div className="form-content">
            ...
            <form onSubmit={handleVerify}>
                ...
                {successMessage && <p className="form-success" style={{color: 'green', textAlign: 'center'}}>{successMessage}</p>}
            ...
            <p className="signup-link">
                Không nhận được mã?{' '}
                <button onClick={handleResendOtp} className="link-button" disabled={resendLoading || resendCooldown > 0}>
                    {resendLoading ? 'Đang gửi...' : (resendCooldown > 0 ? `Gửi lại sau (${resendCooldown}s)` : 'Gửi lại mã')}
                </button>
            </p>
        </div>
    );
