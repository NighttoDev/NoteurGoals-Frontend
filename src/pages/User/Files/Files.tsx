...
const Files: React.FC = () => {
  ...
  const handleFileUpload = async (filesToUpload: FileList) => {
    ...
    for (let i = 0; i < filesToUpload.length; i++) {
      ...
      if (file.name.length > 255) {
        alert(`File name "${file.name}" is too long. Maximum length is 255 characters.`);
        return;
      }
    }
    ...
    const initialProgress = fileNames.reduce((acc, name) => ({ ...acc, [name]: 0 }), {});
  ...
  const handleUnlinkFromGoal = async (fileId: number, goalId: number) => {
    if (!window.confirm('Are you sure you want to unlink this file from the goal?')) return;
    try {
  ...
  const handleUnlinkFromNote = async (fileId: number, noteId: number) => {
    if (!window.confirm('Are you sure you want to unlink this file from the note?')) return;
    try {
      ...
      alert(`Error unlinking file from note: ${error.message}`);
    }
  };
  ...
  const getFileIcon = (fileType: string) => {
    ...
    if (type.includes('word') || type.includes('document')) return <AiOutlineFileWord />;
    if (type.includes('excel') || type.includes('spreadsheet')) return <AiOutlineFileExcel />;
    if (type.includes('zip') || type.includes('archive')) return <AiOutlineFileZip />;
  ...
  const filteredFiles = files
    .filter(file => {
      const matchesType = filterType === 'all' || file.file_type.toLowerCase().includes(filterType);
      return matchesType;
    })
    .sort((a, b) => {
      let comparison = 0;
  ...
  const currentViewLabel = viewOptions.find(option => option.value === viewMode)?.label || 'Grid';
  const currentFilterLabel = filterOptions.find(option => option.value === filterType)?.label || 'All Files';
  const currentSortLabel = sortOptions.find(option => option.value === `${sortBy}-${sortOrder}`)?.label || 'Newest First';
          ...
          <div className="files-action-buttons">
            ...
            <div className="files-filter-dropdown" ref={filterDropdownRef}>
              ...
              >
                ...
                <FontAwesomeIcon 
                  icon={faChevronDown} 
                  className={`files-dropdown-icon ${isFilterDropdownOpen ? 'files-dropdown-icon-open' : ''}`}
                />
              </button>
              {isFilterDropdownOpen && (
                <div className="files-filter-dropdown-menu">
                  {filterOptions.map(option => (
                    <button
                      key={option.value}
                      className={`files-filter-dropdown-item ${filterType === option.value ? 'files-filter-dropdown-item-active' : ''}`}
                      onClick={() => {
            ...
            <div className="files-filter-dropdown" ref={sortDropdownRef}>
              ...
              >
                ...
                <FontAwesomeIcon 
                  icon={faChevronDown} 
                  className={`files-dropdown-icon ${isSortDropdownOpen ? 'files-dropdown-icon-open' : ''}`}
                />
              </button>
              {isSortDropdownOpen && (
                <div className="files-filter-dropdown-menu">
                        ...
                        {file.goals?.map(goal => (
                          <span key={goal.goal_id} className="link-tag">
                            ...
                            <button 
                              onClick={(e) => {
                                e.stopPropagation();
                                handleUnlinkFromGoal(file.file_id, goal.goal_id);
                              }}
                        ...
                        {file.notes?.map(note => (
                          <span key={note.note_id} className="link-tag">
                            ...
                            <button 
                              onClick={(e) => {
                                e.stopPropagation();
                                handleUnlinkFromNote(file.file_id, note.note_id);
                              }}
                              className="unlink-btn"
                            >
                              ×
